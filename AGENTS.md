...
- On each Windows installed primary launch, the content-addressed debugger helper resumes the official process and then remains as its short-lived, event-driven update coordinator; it does not spawn a child that would inherit the App Job. The helper first proves it is outside that Job, then blocks on current-user `PackageCatalog` events and the exact owner process instead of polling. After a successful `OpenAI.Codex` generation change and proven package quiescence, it invokes the existing epoch/registration-checked install transaction. It is not a service, scheduled task, login agent, DOM update-button hook, or second installer. Unrelated packages, stale authorization, running target generations, failed updates, and uncertain identity all fail closed without closing Codex; interrupted replacement retains a private intent that one bounded retry or a later explicit install can consume.
