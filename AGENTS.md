...
- Windows `self-uninstall` accepts only the PowerShell installer's proven current generation. It reuses the managed identity checks and a stable installer lock outside the deleted package tree, then hands deletion to a hidden system PowerShell process after every running CLI from any managed generation exits. Default removal is limited to the two launchers, `packages/standalone`, and the exact CLI bin entry in the current user's PATH; Runtime and session state remain. `--restore-app` first reuses the approved Windows Runtime uninstall and still refuses while Codex is running.
