...
- Profile masking is scoped to the main sidebar identity and its first-level account menu, not the full settings pages. Absence of that surface during navigation is not a mask failure. Re-discover and repair it when it remounts; multiple matching identities or a present identity that cannot be masked still fail validation. Do not inspect or rewrite account details inside settings.
