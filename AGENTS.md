...
- Update Incodex-only code explicitly with `incodex runtime` (writes `~/.incodex/runtime/`, no resign); `inc update` invokes that publisher automatically through the installed CLI. After an official Codex upgrade, the user must run `incodex install` again on the **current** official package.
