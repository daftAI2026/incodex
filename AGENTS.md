...
- `crates/incodex-cli/src/windows_app.rs` owns read-only discovery of the official `OpenAI.Codex` Store package. Trust requires the fixed package family, `Store` signature kind, healthy package status, and the observed Electron executable/ASAR layout; registry guesses and CLI-install paths are not desktop-app evidence.
- `crates/incodex-cli/src/windows_process.rs` owns race-free Windows process-tree containment. It starts the root suspended, assigns it to a kill-on-close Job Object, resumes only after containment, and attributes the IPv4 CDP listener PID back to that Job before injection.
- `crates/incodex-cli/src/windows_open.rs` composes the current user's discovered Store package, private session, isolated Chromium profile, environment, and shared CDP injection options. It must clean or truthfully retain every partially prepared session.
- `crates/incodex-cli/src/cdp.rs` is shared by macOS and Windows. Every debug launch pins Chromium to IPv4 loopback, and every discovered WebSocket endpoint must remain loopback-bound on the allocated port before the committed Runtime injector is sent.
