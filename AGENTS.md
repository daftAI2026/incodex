...
- `crates/incodex-cli/src/windows_process.rs` owns race-free Windows process-tree containment. It starts the root suspended, assigns it to a kill-on-close Job Object, and resumes only after containment so Electron descendants cannot escape through the launch window.
