...
- Automatic Windows rebind preserves the Runtime release selected by the authorized install state or retained repair intent. Validate that release before retiring old registration state, including retry; do not publish the old helper's embedded Runtime or rewrite `runtime/current.json`. Explicit `install` remains the publishing path. Regression tests cover corrupt selected assets, newer Runtime versions, and equal-version/different-hash releases.
