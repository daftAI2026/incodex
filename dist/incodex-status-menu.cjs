...
exports.createNativeStatusMenuBridge = createNativeStatusMenuBridge;
exports.createStatusMenuController = createStatusMenuController;
exports.isCodexStatusMenu = isCodexStatusMenu;
exports.normalizeStatusMenuLabel = normalizeStatusMenuLabel;
...
function createStatusMenuController(options) {
    ...
    function decorate(menu) {
        if (!bridge || !label || !bridge.isCodexStatusMenu(menu))
            return;
        try {
            const item = {
                id: ownItemId(),
...
async function createNativeStatusMenuBridge(options) {
    ...
    const { NobjcClass, NobjcLibrary, callFunction, typedBlock } = await loadObjcModule(appPath);
    ...
    const selector = callFunction("NSSelectorFromString", { returns: ":", args: ["@"] }, selectorName);
    function identifier(value) {
        return NSString.stringWithUTF8String$(value);
    }
    function findItem(menu, id) {
        return (menuItems(menu).find((item) => nativeString(item?.identifier?.()) === id) ?? null);
    }
    function insertItem(menu, index, item) {
        ...
        if (item.type === "separator") {
            ...
            nativeItem = NSMenuItem.alloc().initWithTitle$action$keyEquivalent$(title, selector, empty);
            nativeItem.setTarget$(target);
            nativeItem.setEnabled$(item.enabled !== false);
            if (item.onSelect)
                handlers.set(item.id, item.onSelect);
        }
    ...
    function updateItem(item, update) {
        ...
        if (update.onSelect)
            handlers.set(update.id, update.onSelect);
        else
            handlers.delete(update.id);
    }
    function observeMenuOpen(handler) {
        const notificationName = NSString.stringWithUTF8String$("NSMenuDidBeginTrackingNotification");
