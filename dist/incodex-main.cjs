...
async function attachElectron() {
    ...
    if (windowsPlatform && !isIncognito()) {
        ...
        try {
            normalExitServer = windowsPlatform.listenForNormalExit(`\\\\.\\pipe\\Incodex-Runtime-Control-${registrationId}`, () => electron.app.quit());
            normalExitServer.once("error", (error) => {
                logLaunch("normal-exit-pipe-failed", { error: String(error) });
            });
            electron.app.once("before-quit", () => {
                try {
                    if (normalExitServer?.listening)
                        normalExitServer.close();
                }
                catch {
