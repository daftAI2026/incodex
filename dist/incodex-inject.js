var F=["[data-app-action-sidebar-thread-row]","[data-app-action-sidebar-project-row]"],P=new Set(["Pinned","Recents"]),f=new Set(["Show more","显示更多","展开显示","Show all","显示全部"]),k=new Set(["No chats","没有聊天"]);var m=new Set(["Search","搜索"]);function A(){try{return window.localStorage.getItem("incodex-privacy")==="1"}catch{return!1}}function U(){return window.__incodexIncognito===!0||window.incodex?.isIncognito?.()===!0}function X(z){return z?"退出无痕":"无痕"}function D(z){let G=U(),J=z&&!G;document.documentElement.setAttribute("data-incodex-privacy",J?"on":"off"),document.documentElement.setAttribute("data-incodex-window",G?"incognito":"normal");let K=G||z,Q=document.querySelector("[data-incodex-privacy-toggle]");if(Q)Q.setAttribute("aria-pressed",K?"true":"false"),Q.setAttribute("aria-label",X(K));let V=document.querySelector("[data-incodex-tooltip-label]");if(V)V.textContent=X(K);O(J)}function O(z){for(let G of document.querySelectorAll("[data-app-action-sidebar-section]")){let J=G.getAttribute("data-app-action-sidebar-section-heading")||"";if(z&&P.has(J))G.setAttribute("data-incodex-empty-section","");else G.removeAttribute("data-incodex-empty-section");for(let K of G.querySelectorAll("button")){let Q=(K.textContent||"").replace(/\s+/g," ").trim();if(!f.has(Q))continue;if(z)K.setAttribute("data-incodex-show-more","");else K.removeAttribute("data-incodex-show-more")}for(let K of G.querySelectorAll("span, div, p")){let Q=(K.textContent||"").replace(/\s+/g," ").trim();if(!k.has(Q))continue;if(z)K.setAttribute("data-incodex-empty-chat","");else K.removeAttribute("data-incodex-empty-chat")}}}async function B(){try{if((await window.incodex?.openIncognito?.())?.ok)return!0}catch{}try{return await fetch("https://incodex.invalid/open",{mode:"no-cors",cache:"no-store"}),!0}catch{return!1}}async function M(){if(Z(),U()){try{await window.incodex?.quitIncognito?.()}catch{window.close()}return}await B()}function R(){let z=document.getElementById("incodex-privacy-style");if(!z)z=document.createElement("style"),z.id="incodex-privacy-style",document.head.append(z);let G=F.map((J)=>`html[data-incodex-privacy="on"] ${J}`).join(`,
`);z.textContent=`
  ...
  `,z}function j(){let z=document.querySelector("[data-incodex-landing]");if(!U()||I()){z?.remove();return}let G=L();if(!G)return;let J=T();if(J.parentElement!==G)G.insertBefore(J,G.firstElementChild)}function q(){let z=W();if(!z)return;let G=_(z);if(!G)return;G.setAttribute("data-incodex-header-cluster","true");let J=document.querySelector("[data-incodex-privacy-toggle]");if(!J)J=C(z);if(!x(J,G,z))G.insertBefore(J,G.firstElementChild);D(U()||A())}function w(z){if(!(z.metaKey||z.ctrlKey)||!z.shiftKey)return;if(z.code!=="KeyN"&&z.key.toLowerCase()!=="n")return;z.preventDefault(),z.stopImmediatePropagation(),M()}function N(){if(window.__incodexStarted)return;if(window.__incodexStarted=!0,!U())try{window.localStorage.removeItem("incodex-privacy")}catch{}R(),q(),D(U()),j(),window.addEventListener("keydown",w,!0);let z=!1;new MutationObserver(()=>{if(z)return;z=!0,requestAnimationFrame(()=>{z=!1,q(),j()})}).observe(document.documentElement,{childList:!0,subtree:!0})}if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",N,{once:!0});else N();
