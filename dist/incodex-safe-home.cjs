...
function createSessionHome(userRoot, options = {}) {
    ...
    if (options.handoffPending === true)
        owner.handoffPending = true;
...
function handoffSessionOwner(sessionRoot, pid) {
    ...
    if (owner.handoffPending === true)
        owner.handoffPending = false;
...
function assertBurnIdentity(home, expected) {
    ...
    if (!stats) {
        ...
        if (path.basename(home) === expected.sessionId)
            return;
        throw new Error(`[incodex] missing session owner: ${file}`);
    }
...
function burnSessionHomeInner(target, expected, ownerSnapshot, allowMissingOwner = false) {
    ...
    if (ownerSnapshot) {
        if (allowMissingOwner)
            assertExitedBurnOwner(home, ownerSnapshot);
        else
...
function writeBurnProof(root, expected) {
    if (!expected || !validSessionId(expected.sessionId))
        return false;
    if (!Number.isSafeInteger(expected.ino) || !Number.isSafeInteger(expected.dev))
        return false;
    try {
        ...
        if (!proof)
            return false;
        writePrivateFile(proof, `${JSON.stringify({
...
function readBurnProof(root, userRoot, sessionId) {
    ...
    try {
        ...
        if (!proof || !assertNotSymlink(proof, "burn proof"))
            return null;
    ...
    for (let attempt = 1; attempt <= 5; attempt += 1) {
        ...
        try {
            const removed = childOwner && !originalRemoved
                ...
                : (() => {
                    if (!originalRemoved)
                        assertUnsnapshottedOwner(session.root);
...
function copySettings(home, sourceHome, liveBounds = null) {
    ...
    for (const name of SETTINGS_FILES) {
        ...
        if (!assertNotSymlink(src, "source setting"))
            continue;
...
function validatedWindowBounds(value, requireMaximized = true) {
    ...
    if (x < -0x80000000 || x > 0x7fffffff || y < -0x80000000 || y > 0x7fffffff)
        return null;
    if (width < MAIN_WINDOW_MIN_WIDTH || width > 0x7fffffff)
        return null;
    if (height < MAIN_WINDOW_MIN_HEIGHT || height > 0x7fffffff)
        return null;
    if (requireMaximized && typeof isMaximized !== "boolean")
        return null;
    return { x, y, width, height };
}
// 投影稳定几何，并补回官方因文件预建而跳过的空 Home 初始化哨兵。
function seedWindowState(home, sourceHome, liveBounds = null) {
    const homeStat = assertNotSymlink(home, "session home");
    if (!homeStat?.isDirectory())
        throw new Error(`[incodex] session home missing: ${home}`);
