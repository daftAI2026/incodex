...
exports.isAuxiliarySnapshot = isAuxiliarySnapshot;
...
function classifyWindow(snapshot) {
    ...
    if (AUTH_URL.test(url) || snapshot.hasParent)
        return "main";
    if (snapshot.alwaysOnTop && snapshot.focusable === false)
        return "auxiliary";
    if (snapshot.alwaysOnTop && isSmall(snapshot) && !APP_URL.test(url))
        return "auxiliary";
    return "main";
}
