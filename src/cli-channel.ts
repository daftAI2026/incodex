...
export type UpdateAction = { kind: "refuse"; message: string } | { kind: "reinstall" };
...
export function prefixFromExecPath(execPath: string): string {
  const dir = dirname(execPath);
  return dir.endsWith("/bin") ? dirname(dir) : dir;
}
