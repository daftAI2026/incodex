...
function authorizeSender(snapshot, allowlist) {
  ...
  if (!origin) return { ok: false, code: "URL_NOT_ALLOWED" };
  ...
  if (
    snapshot.frameProcessId !== expected.frameProcessId ||
...
function identityFromWindow(win) {
  ...
  if (!contents || !frame || !origin || contents.isDestroyed?.()) return null;
  return {
...
function bindWindowIdentity(allowlist, win, trustedOrigins) {
  ...
  if (!next || !trustedOrigins?.has?.(next.origin)) {
    if (current) allowlist.set(win.id, { ...current, revoked: true });
    return false;
  }
  if (
    current &&
