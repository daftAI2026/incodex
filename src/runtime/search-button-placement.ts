export type SearchButtonPlacement = {
  parent: HTMLElement;
  before: HTMLElement;
  shellTemplate: HTMLElement | null;
};
...
function isSearchTooltipTrigger(element: HTMLElement): boolean {
  ...
  return element.tagName.toUpperCase() === "SPAN" && state !== null && TOOLTIP_TRIGGER_STATES.has(state);
}
...
export function searchButtonPlacement(search: HTMLElement): SearchButtonPlacement | null {
  ...
  if (isSearchTooltipTrigger(parent) && parent.parentElement) {
    return { parent: parent.parentElement, before: parent, shellTemplate: parent };
  }
  return { parent, before: search, shellTemplate: null };
}
