...
- 官方插件加不了这个按钮，必须改应用包
- 默认安装到官方应用后，改包没法继续保留有效的 OpenAI 签名。下次启动时，macOS 可能要求这个已修改的应用访问钥匙串中的 **Codex Storage Key**。只有对话框里的应用和钥匙串项目都符合预期时，才输入 **Mac 登录密码**（不是 ChatGPT 账号密码）并选择 **始终允许**；否则选择 **拒绝**或**取消**。CLI 不会对 `--clone` 或 `--app` 目标给出永久授权建议
