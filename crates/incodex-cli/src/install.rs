...
pub fn run_install(parsed: &ParsedCli) -> Result<(), String> {
    ...
    crate::confirm::ensure_confirmed("install", parsed.clone || parsed.dry_run || parsed.yes)?;
...
pub fn run_uninstall(parsed: &ParsedCli) -> Result<(), String> {
    ...
    crate::confirm::ensure_confirmed("uninstall", parsed.clone || parsed.dry_run || parsed.yes)?;
    if !app.exists() {
        return Err(format!("Codex app not found: {}", app.display()));
    }
    let mut progress = Progress::new();
