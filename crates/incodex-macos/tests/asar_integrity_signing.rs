...
fn signed_framework_digest_matches_the_updated_plist_without_disabling_validation() {
    let root = std::env::temp_dir().join(format!("incodex-integrity-signing-{}-{}", std::process::id(), SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos()));
    ...
    let bytes = (0..64).step_by(2).map(|i| format!("0x{}", &digest[i..i+2])).collect::<Vec<_>>().join(",");
    ...
    run("clang", &["-dynamiclib", source.to_str().unwrap(), "-o", binary.to_str().unwrap()]);
    let main = root.join("main.c");
    fs::write(&main, "int main(void) {return 0;}").unwrap();
    run("clang", &[main.to_str().unwrap(), "-o", app.join("Contents/MacOS/ChatGPT").to_str().unwrap()]);
    write_plist(&framework.join("Resources/Info.plist"), "Renamed", "com.example.framework", "");
    ...
    write_plist(&app.join("Contents/Info.plist"), "ChatGPT", "com.openai.codex", &integrity);
    run("codesign", &["--force", "--sign", "-", framework.to_str().unwrap()]);
    run("codesign", &["--force", "--sign", "-", app.to_str().unwrap()]);
    ...
    let offset = actual.windows(marker.len()).position(|slice| slice == marker).unwrap();
    let slot = &actual[offset+32..offset+66];
    fs::remove_dir_all(&root).unwrap();
    assert_eq!(&slot[..2], &[1,1], "validation must remain enabled/version 1");
    let expected = "aab91287495cc7a72a1a95c182a0a8be1025e4e8a8057d9812a743981bac2b62";
    let actual_digest = slot[2..].iter().map(|value| format!("{value:02x}")).collect::<String>();
    assert_eq!(actual_digest, expected, "deep codesign success does not prove Electron integrity consistency");
}
