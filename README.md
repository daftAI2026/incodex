...
核 `SHA256SUMS`，把 `incodex` 和 `inc` 放到 `~/.local/bin`。没有 checksum 会失败。需要 [Releases](https://github.com/daftAI2026/incodex/releases) 里已经有对应 tag。
...
需要 [Bun](https://bun.sh) 1.3.14（见 `.bun-version`）和已安装的 Codex / ChatGPT 桌面端。macOS only.
...
`curl … | bash`、`bun link` 都只把命令装到 PATH。改 `/Applications/ChatGPT.app` 的是随后那条 `incodex install`。
