...
description: Audits Incodex changes for installer, signing, asar, session burn, and IPC regressions. Use before merging changes under src/install.ts, src/uninstall.ts, src/codesign.ts, src/asar.ts, src/open-incognito.ts, src/runtime/**, or install.sh.
...
2. Mark every changed sink: asar rewrite, codesign, session create/burn, IPC, PATH installer.
...
7. For `open`, prove no asar/codesign.
